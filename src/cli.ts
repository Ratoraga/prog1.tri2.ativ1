...
import TodoListApelido, { Item, TodoList } from './core'
...
if (command === 'list'){
    const itens = await todolist.getItems()
    console.log('Itens atuais na lista: ')
...
if(command === 'update'){
    ...
    if(!indexStr){
        console.error('Indice do item a ser atualizado não pode ser nulo ou vazio')
        process.exit(1)
    }
    if(!value){
        console.error('Valor do item a ser atualizado não pode ser nulo ou vazio')
        process.exit(1)
    }
    const index = parseInt(indexStr)
    if(isNaN(index)){
    ...
    await todolist.updateItem(index, new Item(value))
    console.log('Item atualizado com sucesso: ', index)
    process.exit(0)
}
